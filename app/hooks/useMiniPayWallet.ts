...
export function useMiniPayWallet() {
  ...
  const connect = useCallback(async () => {
    ...
    try {
      ...
      const raw = e instanceof Error ? e.message : "Wallet connect failed";
      let msg = raw;
      if (/user rejected|denied|ACTION_REJECTED/i.test(raw)) {
        msg = "Connection cancelled in your wallet";
      } else if (/no ethereum|provider not found|wallet not found/i.test(raw)) {
        msg = isMiniPayRuntime()
          ? "MiniPay wallet isn’t ready — reopen the app and try again"
          : "No wallet found — install MetaMask or open Remifi in MiniPay";
      } else if (/returned no accounts/i.test(raw)) {
        msg = "No account shared — unlock your wallet and approve Remifi";
      }
      setState((s) => ({
        ...s,
        connecting: false,
