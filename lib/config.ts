...
const envSchema = z.object({
  ...
  /**
   * Auto payroll on heartbeat (agent signs hire + spends USDC).
   * Set false to stop agent wallet auto activity. Default true.
   */
  HEARTBEAT_SCHEDULES_ENABLED: z
    .string()
    .optional()
    .transform((v) => {
  ...
  /** Telegram bot for settlement / auto-payroll alerts (optional) */
  TELEGRAM_BOT_TOKEN: z.string().optional().transform(emptyToUndef),
  TELEGRAM_BOT_USERNAME: z.string().optional().transform(emptyToUndef),
  TELEGRAM_WEBHOOK_SECRET: z.string().optional().transform(emptyToUndef),
});
