...
/**
 * Settle X-PAYMENT for hire.
 *
 * Track 2 (x402): must go through Celo facilitator — self-settle does not count.
 * Track 1 (tagged volume): hire facilitator txs are untagged; Remifi scores Track 1
 * via classic tagged USDC `transfer` on payroll / wallet sends (not hire EIP-3009).
 */
export async function settleX402Hire(
  ...
  try {
    ...
    console.log(
      "[remifi] x402 hire settled via facilitator (Track 2)",
      extractTxHash(settled),
    );
  ...
  for (const body of fallbacks) {
    try {
      ...
      console.log(
        "[remifi] x402 hire settled via facilitator fallback (Track 2)",
        extractTxHash(settled),
      );
      return { txHash: extractTxHash(settled), raw: settled };
    } catch (err) {
  ...
  // Last resort: agent-tagged EIP-3009 (may help Track 1; usually not Track 2)
  try {
    const { settleX402HireTagged } = await import("./settle-tagged");
    const local = await settleX402HireTagged(paymentHeader, requirements);
    console.warn(
      "[remifi] facilitator settle failed; used agent-tagged settle",
      local.txHash,
    );
    return { txHash: local.txHash, raw: { settlement: "agent_tagged_fallback" } };
  } catch (localErr) {
    console.warn(
      "[remifi] agent-tagged settle also failed",
