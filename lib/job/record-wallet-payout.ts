...
export type RecordWalletPayrollInput = {
  ...
  amount: string;
  payer: string;
  /** Per-recipient proofs (legacy / single-leg). Omit when using fundTxHash. */
  transfers?: WalletTransferInput[];
  /**
   * One tagged USDC transfer from payer → Router for the full payroll amount.
   * Agent then executeSplit — user signs once for multi-recipient payroll.
   */
  fundTxHash?: string;
  clientJobId?: string;
  hire: HireResult;
...
export async function recordWalletPayrollJob(
  ...
  const fundTxHash = input.fundTxHash as Hex | undefined;
  if (fundTxHash) {
    return recordUserFundedRouterPayroll({
      policyId: policy.id,
      amount: input.amount,
      totalAmount,
      payer,
  ...
  if (transfers.length !== legs.length) {
    throw new Error(
      `Expected ${legs.length} transfer proofs, got ${transfers.length}`,
    );
  }
  ...
  try {
    ...
    notifyWalletAsync(
      ...
      settlementSuccessMessage({
        kind: "payroll",
        amount: input.amount,
        recipientCount: proofs.length,
        txHash: firstTransferTxHash(proofs) ?? input.hire.settlementTxHash ?? null,
      }),
    );
    ...
    notifyWalletAsync(
      payer,
      settlementFailMessage({ kind: "payroll", error: message }),
    );
    throw err;
  }
}

async function recordUserFundedRouterPayroll(input: {
  policyId: string;
  amount: string;
  totalAmount: bigint;
  payer: Address;
  ...
  if (!isRouterConfigured()) {
    throw new Error("Router is not configured for one-signature splits");
  }
  const router = getRouterAddress();
  if (!router) throw new Error("ROUTER_ADDRESS is not set");
  ...
  try {
    ...
    notifyWalletAsync(
      ...
      settlementSuccessMessage({
        kind: "payroll",
        amount: input.amount,
        recipientCount: split.transfers.length,
        txHash:
          split.splitTxHash ??
          firstTransferTxHash(split.transfers) ??
          input.hire.settlementTxHash ??
          null,
      }),
    );
    ...
    const message =
      err instanceof Error ? err.message : "Wallet router payroll failed";
    await failPayoutJob(job.id, message);
    notifyWalletAsync(
      input.payer,
      settlementFailMessage({ kind: "payroll", error: message }),
...
export async function recordWalletInstantJob(input: RecordWalletInstantInput) {
  ...
  try {
    ...
    notifyWalletAsync(
      payer,
      settlementFailMessage({ kind: "instant", error: message }),
    );
    throw err;
  }
}
