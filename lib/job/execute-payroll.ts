...
export type ExecutePayrollInput = {
  ...
  clientJobId?: string;
  hire: HireResult;
  /** Telegram alert label; auto for schedule heartbeat */
  notifyKind?: "payroll" | "auto";
};
...
export async function executePayrollJob(
  ...
  try {
    ...
    notifyWalletAsync(
      policy.ownerAddress,
      settlementSuccessMessage({
        kind: input.notifyKind ?? "payroll",
        amount: input.amount,
        recipientCount: transfers.length,
        txHash:
          firstTransferTxHash(transfers) ?? input.hire.settlementTxHash ?? null,
      }),
    );
